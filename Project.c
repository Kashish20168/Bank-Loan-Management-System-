...
float validatePositiveFloat(const char* prompt) 
{
    ...
    while (1)
    {
        ...
        if (*endptr != '\0' || endptr == input || value < 0.0f) 
        {
...
// Validate string input (alphabets + spaces only)
void validateStringInput(const char* prompt, char* buffer, int size) 
{
    while (1)
    {
        ...
        if (strspn(buffer, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz ") 
            != strlen(buffer)) 
        {
...
// Input for address or any free text (allows anything except empty string)
void inputLine(const char* prompt, char* buffer, int size)
{
    while (1)
    {
        printf("%s: ", prompt);
...
{   
    ...
    while (1) 
    {
        ...
        if ((length != requiredLength) || 
            (strspn(buffer, "0123456789") != length)) 
        { 
...
// Validate DOB: accepts "dd mm yyyy", "dd-mm-yyyy", "dd/mm/yyyy" or "ddmmyyyy"
void validateDOB(char* buffer, int minLength, int maxLength) 
{
    while (1) 
    {
        printf("Enter Date of Birth (format dd mm yyyy, dd-mm-yyyy or dd/mm/yyyy): ");
        ...
        {
            printf("Invalid input. DOB must be %d-%d characters long.\n",
                   minLength, maxLength);
            continue;
        }
        
        // Allow digits, spaces, '-' and '/'
        if (strspn(buffer, "0123456789 -/") != length) 
        {
            printf("Invalid input. DOB can only contain digits, spaces, '-' or '/'.\n");
            continue;
        }

        // Normalize to spaces for parsing (replace '-' and '/' with space)
        char temp[20];
        strcpy(temp, buffer);
        int i;
        for (i = 0; temp[i] != '\0'; i++)
        ...
        if (sscanf(temp, "%d %d %d", &day, &month, &year) != 3 || 
            ...
            year < 1900 || year > 2024) 
        {
            printf("Invalid input. Please enter a valid date (dd mm yyyy).\n");
            continue;
        }
        return;
    }
...
void addCustomer() 
{
    ...
    inputLine("Enter Current Address", customer.currentAddress, sizeof(customer.currentAddress));
        
    printf("Enter Email: ");
    fgets(customer.email, sizeof(customer.email), stdin);
    strtok(customer.email, "\n");

    customer.customerNumber = (int)(time(NULL) % 10000);
...
{
    float monthlyRate = loan->interestRate / (12.0f * 100.0f);
    int n = loan->repaymentMonths;

    loan->monthlyInstallment = 
        (loan->loanAmount * monthlyRate * powf(1.0f + monthlyRate, n)) / 
        (powf(1.0f + monthlyRate, n) - 1.0f);
...
int hasActiveLoan(int customerNumber) 
{
    ...
    Loan loan;
    while (fread(&loan, sizeof(Loan), 1, loanFile) == 1) 
    {
        if (loan.customerNumber == customerNumber && 
            strcmp(loan.status, "Approved") == 0 && 
...
void applyForLoan() 
{
    ...
    while (loan.loanAmount > 999999.0f) 
    {
...
void displayLoans() 
{
    ...
    printf("%-20s | %-20s | %-20s | %-20s | %-20s | %-20s | %-20s\n", 
           "Customer No.", "Purpose", "Amount", "Months", 
           "Interest","Installment", "Status");

    while (fread(&loan, sizeof(Loan), 1, file) == 1) 
    {
        printf("%-20d | %-20s | %-20.2f | %-20d | %-20.2f | %-20.2f | %-20s\n", 
            loan.customerNumber, loan.purpose, loan.loanAmount, 
            loan.repaymentMonths, loan.interestRate, 
            loan.monthlyInstallment, loan.status);
    }
    fclose(file);
}
        ...
        if (loan.customerNumber == customerNumber) 
        {
            ...
            while (1)
            {
                ...
                else 
                {
                    ...
                    fseek(loanFile, -(long)sizeof(Loan), SEEK_CUR);
                    fwrite(&loan, sizeof(Loan), 1, loanFile);
                    fflush(loanFile);
...
void menu() 
{
    ...
    do 
    {
        ...
        choice = validatePositiveInt("Enter your choice (1-7)");
        printSeparator();
        ...
        if (choice != 7)
        {
            ...
            getchar();   // wait so user can see output
        }
...
{
    system("COLOR 70");
    header("Bank Loan Management System");
    menu();
    return 0;
}
